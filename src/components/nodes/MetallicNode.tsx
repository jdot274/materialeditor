...
export function MetallicNode({ data, id }: { data: { value: number }, id: string }) {
  const { setMetallic } = useMaterialStore();
  const [inputValue, setInputValue] = useState('0');

  const updateValue = (newValue: number) => {
    const clampedValue = Math.max(0, Math.min(1, newValue));
    data.value = clampedValue;
    setMetallic(clampedValue);
  };
  ...
  }, [id, inputValue, setMetallic]);

  return (
    <div className="bg-white rounded-lg shadow-lg p-4">
      <div className="text-sm font-medium mb-2">Metallic</div>
      <input
        type="number"
        step="0.1"
        min="0"
